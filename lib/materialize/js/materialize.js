...
;(function($) {
  ...
  _generateID = function() {
    _lastID++;
    return 'materialize-modalM-overlay-' + _lastID;
  };
  ...
  var methods = {
    init : function(options) {
      ...
      return this.each(function() {
        var $modalM = $(this);
        var modalM_id = $(this).attr("id") || '#' + $(this).data('target');

        var closemodalM = function() {
          var overlayID = $modalM.data('overlay-id');
          var $overlay = $('#' + overlayID);
          $modalM.removeClass('open');
          ...
          $modalM.find('.modalM-close').off('click.close');
          $(document).off('keyup.modalM' + overlayID);
          ...
          var exitVelocityOptions = {
            ...
            queue: false,
            ease: "easeOutCubic",
            // Handle modalM ready callback
            complete: function() {
              $(this).css({display:"none"});

              // Call complete callback
              if (typeof(options.complete) === "function") {
                options.complete.call(this, $modalM);
              }
              $overlay.remove();
              _stack--;
            }
          };
          if ($modalM.hasClass('bottom-sheet')) {
            $modalM.velocity({bottom: "-100%", opacity: 0}, exitVelocityOptions);
          }
          else {
            $modalM.velocity(
        ...
        var openmodalM = function($trigger) {
          var $body = $('body');
          var oldWidth = $body.innerWidth();
          $body.css('overflow', 'hidden');
          $body.width(oldWidth);

          if ($modalM.hasClass('open')) {
            return;
          }

          var overlayID = _generateID();
          var $overlay = $('<div class="modalM-overlay"></div>');
          var lStack = (++_stack);

          // Store a reference of the overlay
          $overlay.attr('id', overlayID).css('z-index', 1000 + lStack * 2);
          $modalM.data('overlay-id', overlayID).css('z-index', 1000 + lStack * 2 + 1);
          $modalM.addClass('open');
          ...
          if (options.dismissible) {
            ...
            // Return on ESC
            $(document).on('keyup.modalM' + overlayID, function(e) {
          ...
          $modalM.find(".modalM-close").on('click.close', function(e) {
            closemodalM();
          });
          ...
          $modalM.data('associated-overlay', $overlay[0]);
          ...
          var enterVelocityOptions = {
            ...
            complete: function() {
              if (typeof(options.ready) === "function") {
                options.ready.call(this, $modalM, $trigger);
              }
            }
          };
          if ($modalM.hasClass('bottom-sheet')) {
            $modalM.velocity({bottom: "0", opacity: 1}, enterVelocityOptions);
          }
          else {
            $.Velocity.hook($modalM, "scaleX", 0.7);
            $modalM.css({ top: options.startingTop });
            $modalM.velocity({top: options.endingTop, opacity: 1, scaleX: '1'}, enterVelocityOptions);
          }

        };

        // Reset handlers
        $(document).off('click.modalMTrigger', 'a[href="#' + modalM_id + '"], [data-target="' + modalM_id + '"]');
        $(this).off('openmodalM');
        $(this).off('closemodalM');

        // Close Handlers
        $(document).on('click.modalMTrigger', 'a[href="#' + modalM_id + '"], [data-target="' + modalM_id + '"]', function(e) {
          options.startingTop = ($(this).offset().top - $(window).scrollTop()) /1.15;
          openmodalM($(this));
          e.preventDefault();
        }); // done set on click

        $(this).on('openmodalM', function() {
          var modalM_id = $(this).attr("href") || '#' + $(this).data('target');
          openmodalM();
        });

        $(this).on('closemodalM', function() {
          closemodalM();
        });
      }); // done return
    },
    open : function() {
      ...
      $(this).trigger('openmodalM');
    },
    close : function() {
      $(this).trigger('closemodalM');
    }
  };

  $.fn.modalM = function(methodOrOptions) {
    if ( methods[methodOrOptions] ) {
      ...
      $.error( 'Method ' +  methodOrOptions + ' does not exist on jQuery.modalM' );
    }
  };
})(jQuery);
;(function ($) {
  ...
  $.fn.materialbox = function () {
    ...
    return this.each(function() {
      ...
      origin.on('click', function() {
        ...
        // If already modalM, return to original
      ...
      // This function returns the modalMed image to the original spot
 ...
 * Taken from: https://github.com/VodkaBears/RemodalM/blob/master/src/jquery.remodalM.js
 */
